...
export async function alignSlidesToTranscript(
  slideTexts: string[],
  segments: { start: number; end: number; text: string }[],
  totalDurationSeconds: number
): Promise<SlideAlignment[]> {
  if (!slideTexts.length || !segments.length) return [];

  // Build a compact transcript with timestamps for reference
  ...
  const slideSummary = slideTexts
    .map((text, i) => `Slide ${i + 1}: ${text.slice(0, 400).replace(/\s+/g, " ")}`)
    .join("\n");

  const msg = await createWithRetry({
    model: "gpt-4o",
    max_tokens: 1200,
    system: `You align lecture slide content to spoken transcript timestamps. Return ONLY valid JSON, no markdown.`,
    messages: [{
      role: "user",
      content: `Lecture duration: ${totalDurationSeconds} seconds.

SLIDES (${slideTexts.length} total):
${slideSummary}

TRANSCRIPT (with timestamps in seconds):
${timestampedTranscript}

For each slide, find where in the transcript that slide was being presented. Return a JSON array:
[{"slideIndex":0,"startSeconds":0,"endSeconds":45,"description":"Brief description of slide topic"},...]

Rules:
- slideIndex is 0-based
- Timestamps must be within 0 and ${totalDurationSeconds}
- Ranges must not overlap; together they should cover the full lecture
- description is 1 short sentence about the slide's main topic
- Return all ${slideTexts.length} slides`,
    }],
  });

  try {
    const raw = extractText(msg).replace(/```json|```/g, "").trim();
    const parsed = JSON.parse(raw);
    if (!Array.isArray(parsed)) return [];
    return parsed.map((item: any, i: number) => ({
      slideIndex: item.slideIndex ?? i,
      startSeconds: Math.max(0, Number(item.startSeconds) || 0),
      endSeconds: Math.min(totalDurationSeconds, Number(item.endSeconds) || totalDurationSeconds),
      description: item.description ?? "",
    }));
  } catch {
    // Fallback: evenly distribute slides across the lecture duration
    return slideTexts.map((text, i) => ({
      slideIndex: i,
      startSeconds: Math.round((i / slideTexts.length) * totalDurationSeconds),
      endSeconds: Math.round(((i + 1) / slideTexts.length) * totalDurationSeconds),
      description: text.split(/\s+/).slice(0, 8).join(" "),
    }));
  }
}
