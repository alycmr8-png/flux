      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="text-center mb-14">
          ...
          <h2 style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", fontWeight: 800, fontSize: "clamp(32px, 5vw, 50px)", color: "#0f1115", letterSpacing: "-0.03em", lineHeight: 1.1 }}>
            {tr("Your entire semester, remembered.")}
          </h2>
          <p className="mt-4" style={{ color: "rgba(15,17,21,0.7)", fontSize: 18 }}>
            {tr("Start free. Every paid plan includes everything — they only differ on how you pay.")}
          </p>
        </div>
            ...
            return (
              ...
              >
                {(plan.badge || featured) && (
                  ...
                  >
                    {featured ? `${tr("Best value")} · ${tr(plan.badge!)}` : tr(plan.badge!)}
                  </div>
                )}
                ...
                <div className="flex items-end gap-1">
                  ...
                  {plan.period && (
                    <span className="mb-1.5" style={{ fontSize: 16, color: "rgba(15,17,21,0.6)" }}>{tr(plan.period)}</span>
                  )}
                </div>
                <div className="mt-1.5" style={{ fontSize: 15, minHeight: 22, fontWeight: plan.perMonth ? 600 : 400, color: plan.perMonth ? "#4B5FE8" : "rgba(15,17,21,0.6)" }}>
                  {tr(plan.perMonth ?? plan.billing)}
                </div>
                {plan.perMonth && (
                  <div style={{ fontSize: 14, color: "rgba(15,17,21,0.55)" }}>{tr(plan.billing)}</div>
                )}
                ...
                <ul className="flex flex-col gap-3 flex-1 mb-7">
                  {plan.limits.map((l) => (
                    <li key={l} className="flex items-start gap-2.5">
                      ...
                      <span style={{ fontSize: 15.5, color: "rgba(15,17,21,0.8)" }}>{tr(l)}</span>
                    </li>
                  ))}
                </ul>
                ...
                >
                  {plan.id === "free" ? tr("Record 2 lectures free") : tr("Upgrade")}
                </Link>
              </div>
            );
          })}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-4 max-w-4xl mx-auto">
            {PLAN_FEATURES.map((f) => (
              <div key={f} className="flex items-center gap-3">
                ...
                <span style={{ fontSize: 16.5, color: "rgba(15,17,21,0.85)" }}>{tr(f)}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <p className="text-center mt-8" style={{ fontSize: 15, color: "rgba(15,17,21,0.6)" }}>
          {tr("Your first 2 lectures are free · Paid plans are billed today · Cancel anytime")}
        </p>
      </div>
    </section>
  );
