...
function CheatSheetPreview() {
  return (
    <div className="rounded-2xl p-6" style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.08)" }}>
      <div className="text-[10px] uppercase tracking-widest mb-1" style={{ color: "rgba(255,255,255,0.35)" }}>Files</div>
      ...
      <div className="space-y-4">
        {[
          ...
          <div key={i}>
            <div className="text-xs font-semibold mb-2" style={{ color: "white" }}>{section.heading}</div>
            {section.bullets.map((b, j) => (
              <div key={j} className="flex items-start gap-2 mb-1">
                <div className="w-1 h-1 rounded-full mt-1.5 shrink-0" style={{ background: "rgba(255,255,255,0.3)" }} />
                <span className="text-xs leading-relaxed" style={{ color: "rgba(255,255,255,0.6)" }}>{b}</span>
              </div>
            ))}
          </div>
        ))}
        <div>
          <div className="text-xs font-semibold mb-2" style={{ color: "white" }}>Key Terms</div>
          <div className="flex flex-wrap gap-2">
            {["Encoding", "Retrieval", "Mnemonics", "Chunking", "Priming"].map(term => (
              <span key={term} className="text-[10px] px-2.5 py-1 rounded-full" style={{ background: "rgba(255,255,255,0.1)", color: "white" }}>{term}</span>
            ))}
          </div>
        </div>
      </div>
...
function QuizPreview() {
  ...
  return (
    <div className="rounded-2xl p-6" style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.08)" }}>
      <div className="text-[10px] uppercase tracking-widest mb-1" style={{ color: "rgba(255,255,255,0.35)" }}>Question 2 of 8</div>
      <div className="text-sm font-medium leading-relaxed mb-5" style={{ color: "white" }}>
      ...
      <div className="space-y-2 mb-5">
        {["Sensory memory", "Short-term memory", "Long-term memory", "Working memory"].map((opt, i) => (
          <button key={i} onClick={() => setSelected(i)}
            ...
            style={{
              border: `1px solid ${selected === i ? "white" : "rgba(255,255,255,0.1)"}`,
              background: selected === i ? "white" : "rgba(255,255,255,0.05)",
              color: selected === i ? "#111110" : "rgba(255,255,255,0.6)",
            }}>
            <span className="w-3 h-3 rounded-full border shrink-0"
              style={{ borderColor: selected === i ? "#111110" : "rgba(255,255,255,0.3)", background: selected === i ? "#111110" : "transparent" }} />
            {opt}
          </button>
        ))}
      </div>
      <div className="flex items-center justify-between">
        <span className="text-xs" style={{ color: "rgba(255,255,255,0.35)" }}>Auto-generated from your lecture</span>
        <button className="text-xs font-medium px-4 py-2 rounded-full" style={{ background: "white", color: "#111110" }}>Next →</button>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="space-y-3">
      {[
        ...
        <div key={i} className="flex overflow-hidden rounded-2xl" style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.08)" }}>
          ...
          <div className="px-5 py-4 flex-1 flex items-center justify-between gap-4">
            <div>
              <div className="text-sm font-medium mb-1" style={{ color: "white" }}>{book.title}</div>
              <div className="text-xs" style={{ color: "rgba(255,255,255,0.4)" }}>
                {book.chapters} chapters · {book.terms} terms · {book.flashcards} flashcards
              </div>
            </div>
            <div className="text-lg shrink-0" style={{ color: "rgba(255,255,255,0.2)" }}>›</div>
          </div>
        </div>
      ))}
    </div>
...
function VideoPreview() {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-1 p-1 rounded-xl overflow-x-auto" style={{ background: "rgba(37,99,235,0.1)", border: "1px solid rgba(37,99,235,0.15)" }}>
        {vtabs.map(t => (
          <button key={t} onClick={() => setVtab(t.toLowerCase())}
            ...
            style={{ background: vtab === t.toLowerCase() ? "#2563eb" : "transparent", color: vtab === t.toLowerCase() ? "white" : "rgba(255,255,255,0.45)" }}>
            {t}
          </button>
        ))}
      </div>
      ...
      {/* Tab content */}
      <div className="rounded-2xl p-5" style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.08)", minHeight: 280 }}>
                ...
                return (
                  <button key={i} onClick={() => setQuizSelected(i)}
                    ...
                    style={{
                      ...
                      background: picked ? "rgba(37,99,235,0.15)" : "rgba(255,255,255,0.05)",
        ...
        {vtab === "chatbot" && (
          <div className="space-y-3">
            ...
            <div className="flex justify-start">
              <div className="px-4 py-2.5 rounded-2xl rounded-tl-sm text-sm max-w-[85%] leading-relaxed" style={{ background: "rgba(255,255,255,0.06)", color: "rgba(255,255,255,0.75)", border: "1px solid rgba(37,99,235,0.2)" }}>
                At <strong style={{ color: "#60a5fa" }}>0:38</strong>, the video explains: <strong>structuralism</strong> (Wundt) asked <em>what</em> the mind is made of — using introspection to identify basic elements. <strong>Functionalism</strong> (James) asked <em>why</em> the mind works as it does — how mental processes help us adapt and survive.
              </div>
            </div>
            <div className="flex justify-end">
              <div className="px-4 py-2.5 rounded-2xl rounded-tr-sm text-sm max-w-[80%]" style={{ background: "#2563eb", color: "white" }}>
            ...
            <div className="flex justify-start">
              <div className="px-4 py-2.5 rounded-2xl rounded-tl-sm text-sm max-w-[85%] leading-relaxed" style={{ background: "rgba(255,255,255,0.06)", color: "rgba(255,255,255,0.75)", border: "1px solid rgba(37,99,235,0.2)" }}>
                Structuralism came first — Wundt's lab opened in <strong style={{ color: "#60a5fa" }}>1879</strong>. Functionalism emerged shortly after, championed by William James who disagreed with reducing the mind to static elements.
              </div>
            </div>
          </div>
        )}
...
export function FeaturesShowcase() {
  ...
  return (
    <section className="px-6 md:px-16 py-16 md:py-24" style={{ background: "#0a0a0a" }}>
            ...
            {TABS.map(({ key, label, icon: Icon }) => (
              <button key={key} onClick={() => setActive(key)}
                ...
                style={{
                  ...
                  color: active === key ? "white" : "rgba(255,255,255,0.45)",
                }}>
                <Icon size={13} />
                {label}
              </button>
