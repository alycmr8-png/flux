    ...
    <div onClick={() => setFlipped(!flipped)} style={{ perspective: "1000px" }} className="cursor-pointer h-36 relative select-none">
      <div style={{ position: "relative", width: "100%", height: "100%", transformStyle: "preserve-3d", transition: "transform 0.5s cubic-bezier(.4,0,.2,1)", transform: flipped ? "rotateY(180deg)" : "rotateY(0deg)" }}>
        <div style={{ position: "absolute", inset: 0, backfaceVisibility: "hidden", background: "#f5f5f5", border: "1px solid #e5e5e5", borderRadius: 16 }}
          ...
          <p className="text-sm text-black text-center leading-relaxed font-medium">{front}</p>
          <RotateCw size={11} className="text-[#ccc] mt-1" />
        </div>
  ...
  return (
    <div className="rounded-2xl p-5 border" style={{ background: "#fafafa", borderColor: "#ebebeb" }}>
      <div className="flex items-start gap-3 mb-4">
        ...
        <p className="text-sm text-black leading-relaxed font-medium">{index + 1}. {q.question}</p>
      </div>
      ...
      {q.type === "short" && !revealed && (
        <button onClick={() => setRevealed(true)} className="text-sm font-medium text-[#555] hover:text-black transition-colors mb-4 flex items-center gap-1.5">
      ...
      {revealed && (
        <div className="rounded-xl px-4 py-3 mt-2" style={{ background: "#f0f9ff", border: "1.5px solid #bae6fd" }}>
          {q.type === "short" && <p className="text-sm font-semibold text-black mb-1">Answer: <span className="font-normal">{q.correctAnswer}</span></p>}
          <p className="text-xs text-[#555] leading-relaxed">{q.explanation}</p>
        </div>
      )}
      {!revealed && q.type === "mcq" && (
        <button onClick={() => setRevealed(true)} className="text-xs text-[#bbb] hover:text-[#888] transition-colors mt-1">Skip</button>
      )}
    </div>
  );
}
...
// ── Chapter ───────────────────────────────────────────────────────────────────
const CHAPTER_COLORS = ["#3b82f6", "#a855f7", "#22c55e", "#f97316", "#ec4899", "#14b8a6", "#eab308"];
      ...
      {open && (
        <div className="mt-3 space-y-4 pl-1">
          <div className="rounded-2xl p-6 border" style={{ background: "white", borderColor: "#ebebeb" }}>
            <p className="text-sm text-[#444] leading-relaxed whitespace-pre-line">{ch.explanation}</p>
          </div>
              ...
              <div className="space-y-3">
                {ch.keyTerms.map((kt, i) => (
                  <div key={i} className="flex gap-3">
                    ...
                    <div>
                      <span className="text-sm font-semibold text-black">{kt.term}</span>
                      <span className="text-sm text-[#666] ml-2">— {kt.definition}</span>
                    </div>
                  </div>
                ))}
              </div>
          ...
          {ch.analogy && (
            <div className="rounded-2xl p-5 flex gap-4 border" style={{ background: `${color}08`, borderColor: `${color}25` }}>
              ...
              <div>
                ...
                <p className="text-sm text-[#555] leading-relaxed italic">&ldquo;{ch.analogy.analogy}&rdquo;</p>
              </div>
            </div>
          )}
    ...
    <div className="max-w-3xl mx-auto px-8 py-10">
      <div className="flex items-start justify-between mb-10 gap-6">
        ...
        <button onClick={onRegenerate} disabled={regenerating}
          className="flex items-center gap-2 text-sm font-medium px-4 py-2.5 rounded-xl border transition-all hover:border-black hover:text-black disabled:opacity-40 shrink-0"
            ...
            {book.tableOfContents!.map((item, i) => {
              ...
              return (
                <button key={item.chapter}
                  ...
                  className="w-full flex items-center gap-4 px-4 py-3 rounded-xl text-left hover:bg-white transition-all group">
                  <div className="w-6 h-6 rounded-lg flex items-center justify-center text-xs font-bold shrink-0"
                    style={{ background: `${color}18`, color }}>
                    {item.chapter}
                  </div>
                  <span className="flex-1 text-sm text-[#555] group-hover:text-black transition-colors">{item.title}</span>
                  <span className="text-xs font-mono text-[#ccc]">{item.timestamp}</span>
                  <ChevronRight size={13} className="text-[#ccc] group-hover:text-[#888] transition-colors" />
                </button>
              );
            })}
          </div>
        ...
        <div className="mt-8">
          <div className="flex items-center justify-between mb-5">
            ...
            <input value={glossarySearch} onChange={e => setGlossarySearch(e.target.value)}
              ...
              className="bg-[#f5f5f5] border border-[#e5e5e5] rounded-xl px-4 py-2 text-sm text-black placeholder-[#bbb] outline-none focus:border-[#ccc] w-48" />
          </div>
              ...
              <div key={i} className="flex items-start gap-4 px-6 py-4 border-b last:border-0" style={{ borderColor: "#f0f0f0", background: i % 2 === 0 ? "white" : "#fafafa" }}>
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="text-sm font-semibold text-black">{g.term}</span>
                    {g.highYield && (
                      <span className="text-[9px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full"
                        style={{ background: "#fef3c7", color: "#92400e" }}>High yield</span>
                    )}
                  </div>
                  <p className="text-sm text-[#666] leading-relaxed">{g.definition}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="h-full overflow-y-auto" style={{ background: "#f7f7f6" }}>
        <div className="sticky top-0 z-10 flex items-center gap-3 px-8 py-4 border-b" style={{ background: "white", borderColor: "#ebebeb" }}>
          <button onClick={() => setOpenBook(null)}
            className="flex items-center gap-1.5 text-sm text-[#666] hover:text-black transition-colors">
            <ChevronLeft size={14} /> Back
          </button>
          <span className="text-[#ddd]">/</span>
          <span className="text-sm font-medium text-black truncate">{openBook.title}</span>
        </div>
    ...
    <div className="h-full overflow-y-auto" style={{ background: "#f7f5ef" }}>
      <div style={{ maxWidth: 680, margin: "0 auto", padding: "2rem 1.5rem" }}>
        ...
        <div className="flex items-center gap-2.5 mb-5">
          ...
          <h2 className="text-lg font-medium text-black m-0">Study books</h2>
        </div>
          ...
          <div className="flex items-start justify-between gap-4">
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-1.5">
                ...
                <span className="text-sm font-medium text-black">Generate study book</span>
              </div>
          ...
          {showPicker && (
            <div className="mt-4 pt-4 border-t" style={{ borderColor: "rgba(0,0,0,0.07)" }}>
              ...
              <button onClick={() => setUseAll(u => !u)} className="flex items-center gap-2 mb-3">
                {useAll ? <CheckSquare size={14} className="text-black" /> : <Square size={14} style={{ color: "#ccc" }} />}
              ...
              {!useAll && courseId && (
                <div className="space-y-0.5 mb-3 max-h-48 overflow-y-auto">
                  {recordings.map(r => (
                    <label key={r.id} className="flex items-center gap-2 px-2 py-1.5 rounded-lg hover:bg-[#f5f5f5] cursor-pointer">
                      ...
                      <span className="text-xs text-black truncate">{r.title}</span>
                    </label>
                  ))}
                  {notes.map(n => (
                    <label key={n.id} className="flex items-center gap-2 px-2 py-1.5 rounded-lg hover:bg-[#f5f5f5] cursor-pointer">
                      ...
                      <span className="text-xs text-black truncate">{n.name}</span>
                    </label>
                  ))}
                  {files.map(f => (
                    <label key={f.id} className="flex items-center gap-2 px-2 py-1.5 rounded-lg hover:bg-[#f5f5f5] cursor-pointer">
                      ...
                      <span className="text-xs text-black truncate">{f.title}</span>
                    </label>
                  ))}
                </div>
              )}
        ...
        {generating && (
          <div className="rounded-xl p-5 mb-5 flex items-center gap-4 border" style={{ background: "white", borderColor: "rgba(0,0,0,0.08)" }}>
            ...
            <div>
              <p className="text-sm font-medium text-black m-0">Building your study book...</p>
              ...
              {books.map((book, i) => {
                ...
                return (
                  <div key={book.id} className="flex items-center gap-3.5 px-4 py-3.5 rounded-xl border" style={{ background: "white", borderColor: "rgba(0,0,0,0.08)" }}>
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium text-black m-0 truncate">{book.title}</p>
                      <p className="text-xs m-0 mt-0.5" style={{ color: "#6b6a64" }}>{parts.join(" · ")}</p>
                    </div>
                    <button onClick={() => openSavedBook(book)}
                      className="text-xs px-3.5 py-1.5 rounded-lg border transition-all hover:border-black hover:text-black shrink-0"
