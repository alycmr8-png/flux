...
const TYPE_COLOR: Record<string, string> = {
  ...
  Assignment: "bg-indigo-500/10 text-indigo-600 border-indigo-500/20",
  Reading:    "bg-emerald-500/10 text-emerald-600 border-emerald-500/20",
...
function SemesterProgress({ start, end }: { start: string; end: string }) {
  ...
  return (
    <div className="bg-[#FFFFFF] border border-[rgba(0,0,0,0.08)] rounded-2xl p-5 mb-6">
      <div className="flex items-center justify-between mb-3">
        <div>
          <div className="text-[10px] text-gray-600 uppercase tracking-widest mb-0.5">Semester Progress</div>
          <div className="text-sm font-medium text-gray-900">Week {currentWeek} of {totalWeeks}</div>
        </div>
        <div className="text-right">
          <div className="text-2xl font-serif text-gray-900">{Math.round(pct)}%</div>
          <div className="text-[10px] text-gray-600">{daysLeft} days left</div>
        </div>
      </div>
      <div className="h-2 bg-[rgba(0,0,0,0.05)] rounded-full overflow-hidden">
        <div
      ...
      <div className="flex justify-between mt-1.5">
        <span className="text-[10px] text-gray-600">{new Date(start).toLocaleDateString("en-US", { month: "short", day: "numeric" })}</span>
        <span className="text-[10px] text-gray-600">{new Date(end).toLocaleDateString("en-US", { month: "short", day: "numeric" })}</span>
      </div>
    </div>
  );
}
...
function GradeCalculator({ weights }: { weights: { category: string; weight: number }[] }) {
  ...
  return (
    <div className="bg-[#FFFFFF] border border-[rgba(0,0,0,0.08)] rounded-2xl overflow-hidden mb-6">
      <button
        onClick={() => setOpen(o => !o)}
        className="w-full flex items-center justify-between px-5 py-4 hover:bg-black/[0.04] transition-colors"
      >
        <div className="flex items-center gap-2">
          <Star size={13} className="text-gray-600" />
          <span className="text-[10px] text-gray-600 uppercase tracking-widest font-semibold">Grade Calculator</span>
          {projected !== null && (
            <span className="text-[10px] bg-[rgba(0,0,0,0.05)] rounded-full px-2 py-0.5 text-gray-600">
      ...
      {open && (
        <div className="border-t border-[rgba(0,0,0,0.07)] px-5 pb-5 pt-4">
          <div className="space-y-2 mb-4">
            {weights.map(w => (
              <div key={w.category} className="flex items-center gap-3">
                <div className="flex-1 min-w-0">
                  <div className="text-xs text-gray-900 truncate">{w.category}</div>
                  <div className="text-[10px] text-gray-600">{w.weight}% of grade</div>
                </div>
                <div className="flex items-center gap-1.5">
                  <input
                    ...
                    placeholder="—"
                    className="w-16 text-center text-sm border border-[rgba(0,0,0,0.08)] rounded-lg px-2 py-1.5 outline-none focus:border-indigo-500/50 text-gray-900 bg-[#FFFFFF]"
                  />
                  <span className="text-xs text-gray-600">%</span>
                </div>
                {/* Weight bar */}
                <div className="w-16 h-1.5 bg-[rgba(0,0,0,0.05)] rounded-full overflow-hidden">
          ...
          {projected !== null && (
            <div className="border-t border-[rgba(0,0,0,0.07)] pt-3 flex items-center justify-between">
              <div className="text-xs text-gray-600">
                Based on {filled} of {weights.length} categories ({totalWeight}% of grade)
              </div>
              <div className="text-right">
                <div className="text-xl font-serif text-gray-900">{projected.toFixed(1)}%</div>
                <div className="text-xs text-gray-600">{letterGrade(projected)}</div>
              </div>
            </div>
          )}
        </div>
    ...
    <div className="bg-[#FFFFFF] border border-[rgba(0,0,0,0.08)] rounded-2xl overflow-hidden mb-6">
      <div className="px-5 py-4 border-b border-[rgba(0,0,0,0.07)] flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[9px] bg-[rgba(0,0,0,0.05)] rounded-full px-2 py-0.5 text-gray-600">{upcoming.length} upcoming</span>
        </div>
        {past.length > 0 && (
          <span className="text-[10px] text-gray-600">{past.length} past</span>
        )}
      </div>

      {upcoming.length === 0 ? (
        <div className="px-5 py-8 text-center text-sm text-gray-600">No upcoming deadlines.</div>
      ) : (
            ...
            return (
              <div key={i} className="flex items-center gap-4 px-5 py-3.5">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {item.description && (
                    <div className="text-[10px] text-gray-600 truncate mt-0.5">{item.description}</div>
                  )}
                  {item.raw_date_text && (
                    <div className="text-[10px] text-gray-500 mt-0.5 italic">"{item.raw_date_text}"</div>
                  )}
                </div>
      ...
      {upcoming.length > 8 && (
        <div className="px-5 py-3 border-t border-[rgba(0,0,0,0.07)]">
          <button onClick={() => setShowAll(s => !s)} className="text-xs text-gray-600 hover:text-gray-900 transition-colors">
...
function WeeklyGantt({ topics, semesterStart, totalWeeks }: { topics: any[]; semesterStart: string; totalWeeks: number }) {
  ...
  return (
    <div className="bg-[#FFFFFF] border border-[rgba(0,0,0,0.08)] rounded-2xl overflow-hidden mb-6">
      ...
      {open && (
        <div className="border-t border-[rgba(0,0,0,0.07)] px-5 pb-5 pt-4 overflow-x-auto">
          <div className="min-w-[600px]">
            ...
            <div className="flex mb-3">
              ...
              <div className="flex-1 flex">
                {Array.from({ length: totalWeeks }, (_, i) => (
                  <div
                    key={i}
                    className={`flex-1 text-center text-[9px] py-1 rounded-sm ${i + 1 === currentWeek ? "bg-indigo-600 text-white" : "text-gray-600"}`}
                  >
                    {i + 1}
                  </div>
                ))}
            ...
            {topics.slice(0, 16).map((t, i) => (
              <div key={i} className="flex items-center mb-1.5">
                <div className="w-32 shrink-0 text-[10px] text-gray-600 truncate pr-3">{t.title}</div>
                <div className="flex-1 flex">
                  {Array.from({ length: totalWeeks }, (_, wi) => (
                    <div
                      key={wi}
                      className={`flex-1 h-5 rounded-sm mx-px ${wi + 1 === t.week ? "bg-indigo-600" : wi + 1 < (t.week ?? 0) ? "bg-[rgba(0,0,0,0.06)]" : "bg-[rgba(0,0,0,0.06)]"}`}
                    />
                  ))}
                </div>
              </div>
            ...
            <div className="flex mt-3">
              <div className="w-32 shrink-0 text-[10px] text-gray-900 font-medium">Today →</div>
              <div className="flex-1 flex">
                {Array.from({ length: totalWeeks }, (_, i) => (
                  <div key={i} className={`flex-1 h-0.5 ${i + 1 <= currentWeek ? "bg-indigo-600" : "bg-[rgba(0,0,0,0.05)]"}`} />
                ))}
              </div>
            </div>
          </div>
...
export default function SyllabusPage() {
  ...
  return (
    <div className="p-8 max-w-4xl">
      <div style={{ fontSize: 10, fontWeight: 600, letterSpacing: "0.14em", textTransform: "uppercase", color: "rgba(31,35,40,0.5)", marginBottom: 6 }}>
        Syllabus
      </div>
      <h1 style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", fontWeight: 800, fontSize: 30, color: "#1F2328", marginBottom: 6 }}>
        Syllabus Parser
      </h1>
      <p className="text-sm mb-8" style={{ color: "rgba(31,35,40,0.6)" }}>
      ...
      {!result ? (
        <div className="space-y-4 max-w-lg">
          {courses.length > 0 && (
            <div>
              <label className="text-[10px] uppercase tracking-widest block mb-2" style={{ color: "rgba(31,35,40,0.55)" }}>
                Course (optional)
              </label>
              <select
                value={courseId}
                ...
                style={{ background: "rgba(0,0,0,0.05)", border: "1px solid rgba(0,0,0,0.06)", color: "#1F2328" }}
              >
          ...
          <div>
            <label className="text-[10px] uppercase tracking-widest block mb-2" style={{ color: "rgba(31,35,40,0.55)" }}>
              Semester start date <span className="normal-case" style={{ color: "rgba(31,35,40,0.45)" }}>— helps resolve "Week X" dates</span>
            </label>
            <input
              type="date"
              value={semesterStart}
          ...
          <label
            ...
            style={{ borderColor: file ? "rgba(31,35,40,0.5)" : "rgba(0,0,0,0.07)", color: file ? "white" : "rgba(31,35,40,0.55)" }}
          >
            <FileUp size={28} />
            <div className="text-center">
              <div className="text-sm font-medium">{file ? file.name : "Click to upload your syllabus"}</div>
              {!file && <div className="text-xs mt-1" style={{ color: "rgba(31,35,40,0.45)" }}>PDF files only</div>}
            </div>
          ...
          <button
            ...
            style={{ background: "rgba(0,0,0,0.08)", color: "#1F2328" }}
          >
            {loading
        ...
        <div>
          ...
          <div className="flex items-start justify-between mb-6">
            <div>
              <h2 className="text-lg font-medium text-gray-900">{meta?.courseName ?? "Your Course"}</h2>
              <div className="flex items-center gap-3 mt-1">
                {meta?.professor && (
                  <span className="text-xs" style={{ color: "rgba(31,35,40,0.6)" }}>Prof. {meta.professor}</span>
                )}
                {meta?.officeHours && (
                  <span className="text-xs flex items-center gap-1" style={{ color: "rgba(31,35,40,0.6)" }}>
          ...
          {meta?.officeHours && (
            <div className="bg-[#FFFFFF] border border-[rgba(0,0,0,0.08)] rounded-2xl px-5 py-4 flex items-center gap-3">
              <Clock size={13} className="text-gray-600 shrink-0" />
              <div>
                <div className="text-[10px] text-gray-600 uppercase tracking-widest mb-0.5">Office Hours</div>
                <div className="text-sm text-gray-900">{meta.officeHours}</div>
              </div>
            </div>
          )}
        </div>
