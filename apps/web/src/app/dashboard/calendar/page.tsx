...
function countdownColor(days: number, type: string): string {
  if (!URGENT_TYPES.has(type)) return "rgba(31,35,40,0.5)";
  if (days === 0) return "#ef4444";
  if (days <= 1) return "#f97316";
  if (days <= 3) return "#eab308";
  return "rgba(31,35,40,0.5)";
}
  ...
  return (
    <div style={{ color: "#1F2328" }}>
      <style>{`
        ...
        .hour-cell:hover { background: rgba(0,0,0,0.04); }
      `}</style>
      ...
      <div className="flex items-start justify-between mb-6">
        <div>
          ...
          <p style={{ color: "rgba(31,35,40,0.6)", fontSize: 13, marginTop: 2 }}>{t.calendar.subtitle}</p>
        </div>
        {/* View toggle */}
        <div className="flex gap-1 p-1 rounded-xl" style={{ background: "rgba(0,0,0,0.05)", border: "1px solid rgba(0,0,0,0.07)" }}>
          {(["month", "week"] as const).map(v => (
            <button key={v} onClick={() => setView(v)}
              className="px-4 py-1.5 rounded-lg text-sm font-medium transition-all capitalize"
              style={{ background: view === v ? "rgba(0,0,0,0.07)" : "transparent", color: view === v ? "white" : "rgba(31,35,40,0.6)" }}>
          ...
          <div className="flex flex-col gap-1 pl-6">
            {alertDayEvents.map(e => (
              <div key={e.id} className="flex items-center gap-2">
                ...
                <span style={{ fontSize: 12, color: "rgba(31,35,40,0.75)" }}>{e.title}</span>
                <span style={{ fontSize: 10, color: "rgba(31,35,40,0.55)" }}>· {EVENT_TYPES.find(t => t.value === e.type)?.label}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {view === "month" && (
        <div className="flex flex-col lg:flex-row gap-6">
          <div className="flex-1">
            <div className="rounded-2xl p-5" style={{ background: "rgba(0,0,0,0.04)", border: "1px solid rgba(0,0,0,0.07)" }}>
              <div className="flex items-center justify-between mb-5">
                <button onClick={() => setMonth(subMonths(month, 1))} className="p-1.5" style={{ color: "rgba(31,35,40,0.7)" }}><ChevronLeft size={16} /></button>
                <span style={{ fontSize: 14, fontWeight: 500 }}>{format(month, "MMMM yyyy")}</span>
                <button onClick={() => setMonth(addMonths(month, 1))} className="p-1.5" style={{ color: "rgba(31,35,40,0.7)" }}><ChevronRight size={16} /></button>
              </div>
              <div className="grid grid-cols-7 mb-2">
                {["Su","Mo","Tu","We","Th","Fr","Sa"].map(d => (
                  <div key={d} style={{ textAlign: "center", fontSize: 10, color: "rgba(31,35,40,0.5)", paddingBottom: 6 }}>{d}</div>
                ))}
              </div>
                ...
                {days.map(day => {
                  ...
                  return (
                    <button key={day.toISOString()} onClick={() => setSelectedDay(isSelected ? null : day)}
                      ...
                      style={{ background: isSelected ? "rgba(0,0,0,0.08)" : today ? "rgba(0,0,0,0.06)" : "transparent", outline: today ? "1px solid rgba(31,35,40,0.45)" : "none", opacity: past ? 0.4 : 1, minHeight: 52 }}>
                      {/* Day number */}
                      <span className="w-full text-center mb-0.5" style={{ fontSize: 12, fontWeight: today ? 700 : 400, color: today ? "#4B5FE8" : "rgba(31,35,40,0.65)" }}>{format(day, "d")}</span>
                      {/* Event pills */}
                      {de.length > 0 && (
                        <div className="w-full space-y-0.5">
                          ...
                          {de.length > 2 && (
                            <div style={{ fontSize: 9, color: "rgba(31,35,40,0.5)", paddingLeft: 4 }}>+{de.length - 2} more</div>
                          )}
                        </div>
                      )}
                    </button>
            ...
            <div className="flex flex-wrap gap-x-4 gap-y-1 mt-3">
              {EVENT_TYPES.map(t => (
                <div key={t.value} className="flex items-center gap-1.5">
                  ...
                  <span style={{ fontSize: 11, color: "rgba(31,35,40,0.6)" }}>{t.label}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="lg:w-72 flex flex-col gap-4">
            {selectedDay && (
              <div className="rounded-2xl p-4" style={{ background: "rgba(0,0,0,0.04)", border: "1px solid rgba(0,0,0,0.07)" }}>
                ...
                {selectedEvents.length === 0 ? (
                  <p style={{ fontSize: 12, color: "rgba(31,35,40,0.55)" }}>{isPastDay(selectedDay) ? "This day has passed." : "No events — click Add to schedule one."}</p>
                ) : (
            ...
            <div className="rounded-2xl p-4" style={{ background: "rgba(0,0,0,0.04)", border: "1px solid rgba(0,0,0,0.07)" }}>
              <p style={{ fontSize: 11, fontWeight: 600, letterSpacing: "0.1em", textTransform: "uppercase", color: "rgba(31,35,40,0.5)", marginBottom: 12 }}>{t.calendar.upcoming}</p>
              {upcoming.length === 0 ? (
                <p style={{ fontSize: 12, color: "rgba(31,35,40,0.55)" }}>{t.calendar.noEvents}</p>
              ) : (
                <div className="flex flex-col gap-2">
                    ...
                    return (
                      <div key={e.id} className="flex items-start gap-3">
                        <div style={{ textAlign: "right", minWidth: 36 }}>
                          <div style={{ fontSize: 16, fontWeight: 600, lineHeight: 1, color: "#1F2328" }}>{format(new Date(e.date), "d")}</div>
                          <div style={{ fontSize: 9, color: "rgba(31,35,40,0.55)", textTransform: "uppercase" }}>{format(new Date(e.date), "MMM")}</div>
                        </div>
                        <div className="flex-1 rounded-xl px-3 py-2" style={{ background: isUrgentEvent ? "rgba(239,68,68,0.06)" : "rgba(0,0,0,0.04)", borderLeft: `2px solid ${eventColor(e.type)}` }}>
                          <div style={{ fontSize: 12, fontWeight: 500 }}>{e.title}</div>
                          <div className="flex items-center gap-2 mt-1">
                            <span style={{ fontSize: 10, color: "rgba(31,35,40,0.55)" }}>{EVENT_TYPES.find(t => t.value === e.type)?.label}{e.course && ` · ${e.course.code}`}</span>
      ...
      {/* ── WEEK VIEW ── */}
      {view === "week" && (
        <div className="rounded-2xl overflow-hidden" style={{ background: "rgba(0,0,0,0.04)", border: "1px solid rgba(0,0,0,0.07)" }}>
          {/* Week nav */}
          <div className="flex items-center justify-between px-5 py-3" style={{ borderBottom: "1px solid rgba(0,0,0,0.06)" }}>
            <button onClick={() => setWeekStart(subWeeks(weekStart, 1))} className="p-1.5 rounded-lg hover:bg-black/[0.04] transition-colors" style={{ color: "rgba(31,35,40,0.7)" }}><ChevronLeft size={16} /></button>
            <span style={{ fontSize: 14, fontWeight: 500 }}>
              {format(weekStart, "MMM d")} – {format(weekEnd, "MMM d, yyyy")}
            </span>
            <button onClick={() => setWeekStart(addWeeks(weekStart, 1))} className="p-1.5 rounded-lg hover:bg-black/[0.04] transition-colors" style={{ color: "rgba(31,35,40,0.7)" }}><ChevronRight size={16} /></button>
          </div>
          ...
          <div className="overflow-x-auto">
            <div style={{ minWidth: 640 }}>
              {/* Day header row */}
              <div className="grid" style={{ gridTemplateColumns: "52px repeat(7, 1fr)", borderBottom: "1px solid rgba(0,0,0,0.06)" }}>
                <div /> {/* corner */}
                {weekDays.map(day => {
                  const today = isToday(day);
                  return (
                    <div key={day.toISOString()} className="py-2.5 text-center" style={{ borderLeft: "1px solid rgba(0,0,0,0.05)" }}>
                      <div style={{ fontSize: 10, color: today ? "#6E7FF3" : "rgba(31,35,40,0.55)", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.08em" }}>{format(day, "EEE")}</div>
                      <div style={{
                        fontSize: 18, fontWeight: 700, color: today ? "white" : "rgba(31,35,40,0.7)",
              ...
              <div style={{ maxHeight: 540, overflowY: "auto" }}>
                {HOURS.map(hour => (
                  <div key={hour} className="grid" style={{ gridTemplateColumns: "52px repeat(7, 1fr)", borderBottom: "1px solid rgba(0,0,0,0.04)" }}>
                    {/* Hour label */}
                    <div className="py-2 pr-3 text-right" style={{ fontSize: 10, color: "rgba(31,35,40,0.45)", paddingTop: 6, lineHeight: 1 }}>
                      {formatHour(hour)}
                    </div>
                    {/* Day cells */}
                    {weekDays.map(day => {
                        ...
                        <div key={day.toISOString()}
                          ...
                          {evs.map(e => (
                            <div key={e.id}
                              ...
                              {e.course && <div style={{ fontSize: 9, color: "rgba(31,35,40,0.6)", marginTop: 1 }}>{e.course.code}</div>}
                            </div>
                          ))}
                        </div>
                      );
          ...
          {/* Week hint */}
          <div className="px-5 py-3 text-center" style={{ borderTop: "1px solid rgba(0,0,0,0.05)", fontSize: 11, color: "rgba(31,35,40,0.45)" }}>
            {t.calendar.hint}
          </div>
        </div>
      )}
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
          ...
          <div className="w-full max-w-sm rounded-2xl p-6 max-h-[90dvh] overflow-y-auto" style={{ background: "#1a1a18", border: "1px solid rgba(0,0,0,0.08)" }}>
            <div className="flex items-center justify-between mb-5">
              <span style={{ fontSize: 16, fontWeight: 500 }}>{editingEvent ? t.calendar.editEventTitle : t.calendar.addEventTitle}</span>
              <button onClick={() => setShowModal(false)} style={{ color: "rgba(31,35,40,0.6)" }}><X size={16} /></button>
            </div>
            ...
            <div className="flex flex-col gap-3">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {EVENT_TYPES.map(t => (
                    <button key={t.value} onClick={() => setType(t.value)}
                      ...
                      style={{
                        background: type === t.value ? t.color : "rgba(0,0,0,0.05)",
                        color: type === t.value ? "white" : "rgba(31,35,40,0.7)",
                        border: `1px solid ${type === t.value ? t.color : "rgba(0,0,0,0.07)"}`,
                      }}>
                      {t.label}
                    </button>
                  ))}
              ...
              <button onClick={saveEvent} disabled={saving || !title.trim() || !date}
                ...
                style={{ background: saving || !title.trim() || !date ? "rgba(0,0,0,0.07)" : "white", color: saving || !title.trim() || !date ? "rgba(31,35,40,0.6)" : "black", cursor: saving || !title.trim() || !date ? "not-allowed" : "pointer" }}>
...
const labelStyle: React.CSSProperties = {
  ...
  textTransform: "uppercase", color: "rgba(31,35,40,0.6)",
...
function EventCard({ event, onEdit, onDelete }: { event: any; onEdit: () => void; onDelete: () => void }) {
  ...
  return (
    <div className="flex items-start justify-between rounded-xl px-3 py-2.5" style={{ background: "rgba(0,0,0,0.05)", borderLeft: `3px solid ${eventColor(event.type)}` }}>
      <div className="flex-1 min-w-0">
        <div style={{ fontSize: 13, fontWeight: 500 }}>{event.title}</div>
        <div className="flex items-center gap-2 mt-1">
          <span style={{ fontSize: 11, color: "rgba(31,35,40,0.6)" }}>
        ...
        {event.description && <div style={{ fontSize: 11, color: "rgba(31,35,40,0.55)", marginTop: 3 }}>{event.description}</div>}
      </div>
