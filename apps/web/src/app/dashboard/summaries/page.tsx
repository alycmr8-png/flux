...
function ScheduleButton({ lectureId }: { lectureId: string }) {
  ...
  return (
    <div className="relative">
      ...
      {open && (
        <div className="absolute right-0 top-8 z-10 rounded-xl p-3 w-52 shadow-2xl border" style={{ background: "#1a1a1a", borderColor: "rgba(255,255,255,0.1)" }}>
          <p className="text-[10px] mb-2" style={{ color: "rgba(255,255,255,0.4)" }}>Optional: set your exam date</p>
          <input
            type="date"
            value={examDate}
  ...
  return (
    <div className="p-8 max-w-2xl">
      ...
      {!isLoading && !sheets.length && (
        <p className="text-sm" style={{ color: "rgba(255,255,255,0.4)" }}>No cheat sheets yet. Record a lecture to get started.</p>
      )}

      <div className="space-y-4">
        {sheets.map((cs: any) => (
          <div key={cs.id} className="rounded-2xl p-5 border" style={{ background: "rgba(255,255,255,0.06)", borderColor: "rgba(255,255,255,0.08)" }}>
            <div className="flex items-start justify-between mb-4">
              <div>
                <div className="font-medium text-sm" style={{ color: "white" }}>{cs.title}</div>
                {cs.lecture?.course?.code && (
                  <div className="text-xs mt-0.5" style={{ color: "rgba(255,255,255,0.35)" }}>{cs.lecture.course.code}</div>
                )}
              </div>
            ...
            {cs.content?.actionItems?.length > 0 && (
              <div className="rounded-xl p-3 mb-4 border" style={{ background: "rgba(255,255,255,0.04)", borderColor: "rgba(255,255,255,0.07)" }}>
                <div className="flex items-center gap-1.5 mb-2.5">
                  <AlarmClock size={10} style={{ color: "rgba(255,255,255,0.6)" }} />
                  <span className="text-[9px] font-bold uppercase tracking-widest" style={{ color: "rgba(255,255,255,0.6)" }}>Action items</span>
                </div>
                  ...
                  {cs.content.actionItems.map((item: any, i: number) => {
                    ...
                    return (
                      <div key={i} className="flex items-start gap-2 text-xs" style={{ color: "rgba(255,255,255,0.45)" }}>
                        <Icon size={11} className="shrink-0 mt-0.5" style={{ color: "rgba(255,255,255,0.35)" }} />
                        <span>{item.text}{item.dueDate ? <span style={{ color: "rgba(255,255,255,0.35)" }}> — {item.dueDate}</span> : ""}</span>
                      </div>
                    );
                  })}
                </div>
            ...
            {cs.content?.sections?.slice(0, 2).map((s: any, i: number) => (
              <div key={i} className="mb-3">
                <div className="text-[10px] font-bold uppercase tracking-widest mb-2" style={{ color: "rgba(255,255,255,0.35)" }}>{s.heading}</div>
                <div className="space-y-1">
                  {s.bullets?.slice(0, 3).map((b: string, j: number) => (
                    <div key={j} className="flex gap-2 text-xs" style={{ color: "rgba(255,255,255,0.5)" }}>
            ...
            {cs.content?.keyTerms?.length > 0 && (
              <div className="mb-3">
                <div className="text-[10px] font-bold uppercase tracking-widest mb-2" style={{ color: "rgba(255,255,255,0.35)" }}>Key Terms</div>
                <div className="space-y-1">
                  {cs.content.keyTerms.slice(0, 4).map((kt: any, i: number) => (
                    <div key={i} className="text-xs">
                      <span className="font-medium" style={{ color: "white" }}>{kt.term}</span>
                      <span style={{ color: "rgba(255,255,255,0.45)" }}> — {kt.definition}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {cs.content?.formulas?.length > 0 && (
              <div className="mb-3">
                <div className="text-[10px] font-bold uppercase tracking-widest mb-2" style={{ color: "rgba(255,255,255,0.35)" }}>Formulas</div>
                <div className="space-y-1">
                  {cs.content.formulas.map((f: string, i: number) => (
                    <div key={i} className="text-xs font-mono" style={{ color: "rgba(255,255,255,0.4)" }}>{f}</div>
                  ))}
                </div>
              </div>
            )}
            ...
            {cs.content?.examTips?.[0] && (
              <div className="rounded-xl p-3 mt-2 border" style={{ background: "rgba(255,255,255,0.04)", borderColor: "rgba(255,255,255,0.07)" }}>
                <div className="text-[9px] font-bold uppercase tracking-widest mb-1" style={{ color: "rgba(255,255,255,0.5)" }}>Exam tip</div>
                <div className="text-xs leading-relaxed" style={{ color: "rgba(255,255,255,0.45)" }}>{cs.content.examTips[0]}</div>
              </div>
            )}
          </div>
        ))}
